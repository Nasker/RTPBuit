...
uint8_t BassSequence::_computeSlot() {
    ...
    uint8_t totalSlots = arpCount * BASS_OCTAVES;
...
void BassSequence::_retriggerLiveNote() {
    ...
    int note = BASS_BASE_NOTE + pitchClass + interval + zone * 12;
...
void BassSequence::handleLiveSequencerTick() {
    // 16th note tick - not used for rolls (they're on half-ticks)
}

void BassSequence::handleLiveHalfTick() {
    _tickCount++;
